...
def process_csv(weblog_data):
    """
    Processes the CSV file, and returns a count of total rows, and total image files.
    ...
    return {'image_count': image_count, 'total_rows': total_rows}


def find_image_percentage(image_count: int, total_rows: int):
    """
    Checks for .jpg, .gif, .png files extensions in web search hits and returns
    the percentage of images found. 

    :return: Image requests account for (x)% of all requests
    """
    percentage: float = (image_count / total_rows) * 100
    return percentage

def find_most_used_browser(weblog_data):
    """
...
def main():
    ...
    # Process csv data
    results = process_csv(weblog_data)
    image_count = results['image_count']
    total_rows = results['total_rows']

    percentage = find_image_percentage(image_count, total_rows)

    print(f"Found {image_count} images out of {total_rows} total.")
    print(f"Image request account for {percentage:.2f}% of all requests in data.")
